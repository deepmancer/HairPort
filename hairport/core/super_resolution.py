...
# NOTE: PathConfig was previously imported from a non-existent config package.
# It is imported but never actually used in this file.
...
class CodeFormerEnhancer:
    ...
    def __init__(
        ...
        bg_tile: int | None = None,
        bg_tile_pad: int | None = None,
        ...
        self.device = torch.device(_device if torch.cuda.is_available() else "cpu")
        self.ultrasharp = ultrasharp

        self.CODEFORMER_ROOT = Path(cfg.paths.codeformer_module)
        ...
        self._init_realesrgan(
            bg_tile if bg_tile is not None else cfg.codeformer.bg_tile,
            bg_tile_pad if bg_tile_pad is not None else cfg.codeformer.bg_tile_pad,
        )

        # init CodeFormer network
        self._init_codeformer(
            face_size if face_size is not None else cfg.codeformer.face_size
        )
