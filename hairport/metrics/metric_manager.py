...
BASE_DIR = "/workspace/celeba_reduced"
...
PATHS = {
    ...
    "source_hair_mask": {
        "annot_dir": "/workspace/CelebAMask-HQ/CelebAMask-HQ-mask-anno",
        "filename": "{source_id}_hair.png",
    },
    "reference_hair_mask": {
        "annot_dir": "/workspace/CelebAMask-HQ/CelebAMask-HQ-mask-anno",
        "filename": "{target_id}_hair.png",
    },
    # Baseline methods (grouped under 'baselines' key)
    'baselines': {
        'hairfastgan': {
            "base_dir": f"{BASE_DIR}/baselines/hairfastgan/",
            "transferred": f"{BASE_DIR}/baselines/hairfastgan/{{target_id}}_to_{{source_id}}/transferred.png",
            "hair_mask": f"{BASE_DIR}/baselines/hairfastgan/{{target_id}}_to_{{source_id}}/hair_mask.png",
        },
        'stablehair': {
            "base_dir": f"{BASE_DIR}/baselines/stablehair/",
            "transferred": f"{BASE_DIR}/baselines/stablehair/{{target_id}}_to_{{source_id}}/transferred.png",
            "hair_mask": f"{BASE_DIR}/baselines/stablehair/{{target_id}}_to_{{source_id}}/hair_mask.png",
        },
        'hairclipv2': {
            "base_dir": f"{BASE_DIR}/baselines/hairclipv2/",
            "transferred": f"{BASE_DIR}/baselines/hairclipv2/{{target_id}}_to_{{source_id}}/transferred.png",
            "hair_mask": f"{BASE_DIR}/baselines/hairclipv2/{{target_id}}_to_{{source_id}}/hair_mask.png",
        },
        'hairclip': {
            "base_dir": f"{BASE_DIR}/baselines/hairclip/",
            "transferred": f"{BASE_DIR}/baselines/hairclip/{{target_id}}_to_{{source_id}}/transferred.png",
            "hair_mask": f"{BASE_DIR}/baselines/hairclip/{{target_id}}_to_{{source_id}}/hair_mask.png",
        },
        'barbershop': {
            "base_dir": f"{BASE_DIR}/baselines/barbershop/",
            "transferred": f"{BASE_DIR}/baselines/barbershop/{{target_id}}_to_{{source_id}}/transferred.png",
            "hair_mask": f"{BASE_DIR}/baselines/barbershop/{{target_id}}_to_{{source_id}}/hair_mask.png",
        },
        'hairfusion': {
        ...
        'styleyourhair': {
            "base_dir": f"{BASE_DIR}/baselines/styleyourhair/",
            "transferred": f"{BASE_DIR}/baselines/styleyourhair/{{target_id}}_to_{{source_id}}/transferred.png",
            "hair_mask": f"{BASE_DIR}/baselines/styleyourhair/{{target_id}}_to_{{source_id}}/hair_mask.png",
        },
        # 'FluxKlein_Hi3DGen_3d_unaware': {
    ...
    'hairport': {
        ...
        'Ours (FLUX.2 Kleinn)': {
            "base_dir": f"{BASE_DIR}/view_aligned/shape_hi3dgen__texture_mvadapter",
            "transferred": f"{BASE_DIR}/view_aligned/shape_hi3dgen__texture_mvadapter/{{target_id}}_to_{{source_id}}/w_seg/{{awareness}}/transferred_kleinn/hair_restored.png",
            "hair_mask": f"{BASE_DIR}/view_aligned/shape_hi3dgen__texture_mvadapter/{{target_id}}_to_{{source_id}}/w_seg/{{awareness}}/transferred_kleinn/hair_restored_mask.png",
        },
        'Ours (FLUX.2 - Direct3DS2)': {
            "base_dir": f"{BASE_DIR}/view_aligned/shape_hi3dgen__texture_mvadapter",
            "transferred": f"{BASE_DIR}/view_aligned/shape_direct3d_s2__texture_mvadapter/{{target_id}}_to_{{source_id}}/w_seg/{{awareness}}/transferred_klein/hair_restored.png",
            "hair_mask": f"{BASE_DIR}/view_aligned/shape_direct3d_s2__texture_mvadapter/{{target_id}}_to_{{source_id}}/w_seg/{{awareness}}/transferred_klein/hair_restored_mask.png",
        },
        'Ours (FLUX.2, W.O Segmentation)': {
            "base_dir": f"{BASE_DIR}/view_aligned/shape_hi3dgen__texture_mvadapter",
...
def _find_source_hair_mask(source_id: str, annot_dir: str, filename_pattern: str) -> str:
    ...
    pattern = os.path.join(annot_dir, "*", filename)
    matches = glob.glob(pattern)
    if not matches:
        raise FileNotFoundError(f"Source hair mask not found: checked {BASE_DIR}/hair_mask/{source_id}.* and {pattern}")
    return matches[0]
...
def _load_sample(
    ...
    source_img = Image.open(source_path).convert("RGB").resize((1024, 1024), Image.LANCZOS)
    
    # Load reference image
    reference_path = paths["reference_image"].format(target_id=target_id)
    if not os.path.exists(reference_path):
        raise FileNotFoundError(f"Reference image not found: {reference_path}")
    reference_img = Image.open(reference_path).convert("RGB").resize((1024, 1024), Image.LANCZOS)
    ...
    generated_img = Image.open(generated_path).convert("RGB").resize((1024, 1024), Image.LANCZOS)
    ...
    source_hair_mask = Image.open(source_mask_path).convert("L").resize((1024, 1024), Image.NEAREST)
    ...
    generated_hair_mask = Image.open(generated_mask_path).convert("L").resize((1024, 1024), Image.NEAREST)
    ...
    if "reference_hair_mask" in paths:
        ...
        if ref_mask_path is not None:
            reference_hair_mask = Image.open(ref_mask_path).convert("L").resize((1024, 1024), Image.NEAREST)
...
if __name__ == "__main__":
    results_dict, global_metrics_dict = evaluate_methods(
        ...
        batch_size=32,
        to_include_angle_threshold=0.20,  # Only include pairs with head_diff_angle > 0.15 radians
    )
    
    results_dict, global_metrics_dict = generate_comparisons(
        output_dir="/workspace/celeba_reduced/evaluation",
        methods="all",
        metrics=['ssim_nonhair_intersection', 'psnr_nonhair_intersection', 'clip_i', 'fid', 'fid_clip', 'ids', 'dinov3_hair_similarity', 'dinov2_hair_similarity', 'lpips'],
    )
