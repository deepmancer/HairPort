...
class StableDiffusion:
    ...
    def __init__(
        ...
        # Pre-initialized masks for different resolutions
        # 2x2 mode (square images)
        self.mask1_1024 = torch.zeros((1,1,1024,1024))
        self.mask1_1024[:,:,self.mask1_1024.size(2)//2:,self.mask1_1024.size(3)//2:] = 1
        
        self.mask1_2048 = torch.zeros((1,1,2048,2048))
        self.mask1_2048[:,:,self.mask1_2048.size(2)//2:,self.mask1_2048.size(3)//2:] = 1
        
        # 2x1 mode (width = 2*height)
        self.mask2_1024 = torch.zeros((1,1,512,1024))
        self.mask2_1024[:,:, : ,self.mask2_1024.size(3)//2:] = 1
        
        self.mask2_2048 = torch.zeros((1,1,1024,2048))
        self.mask2_2048[:,:, : ,self.mask2_2048.size(3)//2:] = 1
                ...
                for i in tqdm(range(len(image_configs)), desc=f"Generating Images", leave=False):
                    ...
                    if self.is_xl:
                        ...
                        if self.model_config.use_flux_cfg:
                            ...
                            im_ = Image.open(gen_config.image).convert('RGB')
                            gen_config.width, gen_config.height = im_.size
                            if gen_config.height == gen_config.width:
                                # print(im_.size, "line 1365 stable_diffusion_model_inpaint.py")
                                # print(gen_config.width, gen_config.height)
                                im_mask = Image.open("mask1.png")
                                if gen_config.width == 2048:
                                    h, w = im_mask.size
                                    im_mask = im_mask.resize((w*2, h*2), Image.NEAREST)
                            elif gen_config.height*2 == gen_config.width:
                                im_mask = Image.open("mask2.png")
                            img = pipeline(
                                image=im_,
                                mask_image=im_mask,
                                prompt_embeds=conditional_embeds.text_embeds,
    ...
    def add_noise(
        ...
        # Select appropriate mask based on image mode and resolution
        latent_h = original_samples_chunks[0].shape[2]
        latent_w = original_samples_chunks[0].shape[3]
        
        if latent_h == latent_w:
            # print('line 1617 stable_diffusion_model_inpaint.py', latent_h, latent_w)
            # 2x2 mode (square): select mask based on resolution
            if latent_h <= 128:  # 1024x1024 image → 128x128 latent
                mask = self.mask1_1024.to(self.device_torch, dtype=torch.float16).detach()
            else:  # 2048x2048 image → 256x256 latent
                mask = self.mask1_2048.to(self.device_torch, dtype=torch.float16).detach()
        elif latent_w == 2 * latent_h:
            # 2x1 mode (width = 2*height): select mask based on resolution
            if latent_h <= 64:  # 1024x2048 image → 128x256 latent
                mask = self.mask2_1024.to(self.device_torch, dtype=torch.float16).detach()
            else:  # 2048x4096 image → 256x512 latent
                mask = self.mask2_2048.to(self.device_torch, dtype=torch.float16).detach()
            ...
            if self.is_pixart:
                ...
                if self.is_flux:
                    with torch.no_grad():
                        ...
                        if h == w:
                            img_ids[..., 0] = 1
                            img_ids[h//4:, w//4:, 0] = 0  # inpaint region - bottom-right quadrant (packed coords)
                        elif w == 2*h:
                            img_ids[..., 0] = 1
                            img_ids[:, w//4:, 0] = 0  # inpaint region - right half (packed coords)
