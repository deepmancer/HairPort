...
class CaptionMixin:
    def get_caption_item(self: 'AiToolkitDataset', index):
        ...
        # handle replacements
        replacement_list = self.dataset_config.replacements if isinstance(self.dataset_config.replacements,
                                                                          list) else []
...
class ClipImageFileItemDTOMixin:
    ...
    def build_clip_imag_augmentation_transform(self: 'FileItemDTO'):
        if self.dataset_config.clip_image_augmentations is not None and len(
                self.dataset_config.clip_image_augmentations) > 0:
...
class AugmentationFileItemDTOMixin:
    ...
    def augment_image(self: 'FileItemDTO', img: Image, transform: Union[None, transforms.Compose], ):
        ...
        augmented_params['transforms'] = [t for t in augmented_params['transforms'] if
                                          t['__class_fullname__'].split('.')[-1] in spatial_transforms]
...
class LatentCachingMixin:
            ...
            for file_item in tqdm(self.file_list, desc=f'Caching latents{" to disk" if to_disk else ""}'):
                ...
                if os.path.exists(latent_path):
                    ...
                    try:
                        ...
                        # Check if we need split encoding for 2x2 or 2x1 modes
                        if h == w:
                            # 2x2 mode (4-panel grid)
                            h_half, w_half = h // 2, w // 2
                            parts = []
                            parts.append(imgs[..., :h_half, :w_half])  # top-left
                            parts.append(imgs[..., :h_half, w_half:])  # top-right
                            parts.append(imgs[..., h_half:, :w_half])  # bottom-left
                            parts.append(imgs[..., h_half:, w_half:])  # bottom-right
                            
                            encoded_parts = [self.sd.encode_images(part) for part in parts]
                            
                            top = torch.cat([encoded_parts[0], encoded_parts[1]], dim=-1)
                            bottom = torch.cat([encoded_parts[2], encoded_parts[3]], dim=-1)
                            latent = torch.cat([top, bottom], dim=-2).squeeze(0)
                        elif w == h * 2:
                            # 2x1 mode (2-panel side-by-side)
                            w_half = w // 2
                            parts = []
                            parts.append(imgs[..., :, :w_half])  # left panel
                            parts.append(imgs[..., :, w_half:])  # right panel
                            
                            encoded_parts = [self.sd.encode_images(part) for part in parts]
                            
                            latent = torch.cat([encoded_parts[0], encoded_parts[1]], dim=-1).squeeze(0)
                        else:
                            # Standard encoding
    ...
    def cache_latents_all_latents2(self: 'AiToolkitDataset'):
        print("cache_latents_all_latents2!")
        with accelerator.main_process_first():
            print_acc(f"Caching latents for {self.dataset_path}")
            # cache all latents to disk
            to_disk = self.is_caching_latents_to_disk
            ...
            for file_item in tqdm(self.file_list, desc=f'Caching latents{" to disk" if to_disk else ""}'):
                ...
                if os.path.exists(latent_path):
                    ...
                    try:
                        ...
                        if h == w:
                            # 2x2 mode (4-panel grid)
                            print(f"2x2 mode detected: {h}x{w}")
                            h_half, w_half = h // 2, w // 2
                            parts = []
                            parts.append(imgs[..., :h_half, :w_half])  # top-left
                            parts.append(imgs[..., :h_half, w_half:])  # top-right
                            ...
                            # 2x1 mode (2-panel side-by-side)
                            print(f"2x1 mode detected: {h}x{w}")
                            w_half = w // 2
                            parts = []
                            parts.append(imgs[..., :, :w_half])  # left panel
                            parts.append(imgs[..., :, w_half:])  # right panel
