...
class CustomFlowMatchEulerDiscreteScheduler(FlowMatchEulerDiscreteScheduler):
    ...
    def add_noise_mask3(
        ...
        # Mode detection: check if 2x2 or 2x1
        if original_samples.size(2) == original_samples.size(3):
            # 2x2 mode: mask bottom-right quadrant
            mask = torch.zeros_like(original_samples)
            mask[:,:,original_samples.size(2)//2:,original_samples.size(3)//2:] = 1
        elif original_samples.size(3) == original_samples.size(2) * 2:
            # 2x1 mode: mask right half
            mask = torch.zeros_like(original_samples)
            mask[:,:,:,original_samples.size(3)//2:] = 1
        else:
            # Standard mode: no masking
            mask = torch.ones_like(original_samples)
    ...
    def add_noise2(
        ...
        # noisy_sample = (1 - timestep) * latent + timestep * noise
        t_01 = (timesteps / 1000).to(original_samples.device)
        noisy_model_input = original_samples.clone()
        
        # Mode detection: check if 2x2 or 2x1
        if original_samples.size(2) == original_samples.size(3):
            # 2x2 mode: add noise to bottom-right quadrant only
            ...
            # 2x1 mode: add noise to right half only
            right_half = original_samples[:,:,:,original_samples.size(3)//2:]
            noisy_right_half = (1 - t_01) * right_half + t_01 * noise
            noisy_model_input[:,:,:,noisy_model_input.size(3)//2:] = noisy_right_half
        else:
            # Standard mode: add noise to everything
            noisy_model_input = (1 - t_01) * original_samples + t_01 * noise
    ...
    def add_masked_noise(
        ...
        if original_samples.size(2) == original_samples.size(3):
            ...
            # 2x1 mode: add noise to right half only
            right_half = original_samples[:, :, :, original_samples.size(3) // 2:]
            right_half_noisy = (1 - t_01) * right_half + t_01 * noise
            noisy_model_input[:,:,:,noisy_model_input.size(3)//2:] = right_half_noisy
        else:
            # Standard mode: add noise to everything
            noisy_model_input = (1 - t_01) * original_samples + t_01 * noise
